...
<script setup lang="ts">
    import { ref } from 'vue';
    import { useUpdateProductCategoryCommand } from '@/commands/products/updateProductCategoryCommand';
    ...
    const selectedProductCategory = ref<ProductCategoryDto>();
    ...
    const updateProductCategoryCommand = useUpdateProductCategoryCommand(
        (errorMessage) => toastService.showError(`Error while creating category: ${errorMessage}`),
        () => emit('onUpdate'),
    );

    const updateProductCategoryFormSchema = yup.object({
    ...
    const onSubmit = handleSubmit(async (values) => {
        await updateProductCategoryCommand(
            props.id, 
            {
              name: values.name,
    ...
    function getSubcategories(subcategoryNames: string[]) {
      return subcategoriesOptions.value?.filter((x) => subcategoryNames.includes(x.name));
    }
</script>
